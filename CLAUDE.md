...
**Home = guia-diretório (`app/(site)/page.tsx`).** A `/` NÃO é mais a landing multi-concurso: é o guia do candidato no modelo de `site/index.html` - hero (nome do concurso + resumo + 4 stats) + `ConcursoSeletor` (`/?c=<slug>`) + grade de cargos (`CargoLista`), tudo do banco. Planos + `FeatureGrid` em `/planos`; FAQ em `/duvidas`. Nav: Vagas (`/`) · Simulados · Ranking · Planos · Dúvidas. `ConcursoExplorer`/`ConcursoCarousel`/`VagasFiltro`/`ConteudoSection`/`EditalSection` viraram código morto.

**Guia de concurso no banco (`lib/concurso-guia.ts` + `prisma/seed-concursos.ts`).** Modelos `Concurso`, `CargoConcurso`, `MateriaConcurso`, `ItemEstudo` + `Simulado.concursoId`. Seed a partir de `prisma/dados/<slug>/` (JSON extraído do edital / de `site/` pelo "Claude Desktop", ver `INTEGRACAO-PETROPREP.md`). **ses-to-2026** (Saúde Tocantins, FGV): 72 cargos, 8 matérias, ~3,2k itens (102 com resumo - piloto). **transpetro-2026**: 61 cargos (de `site/index.html` + `site/vagas/*.html`: requisito/finalidade/atribuições/remuneração), 2 matérias (Língua Portuguesa, Matemática), itens de `site/topicos/*.html` (31 tópicos: `textoOficial`/`resumo`("O que é")/`comoFunciona`/`exemplos[]`). Rotas `/concurso/[slug]/**` (`vaga/[cargo]`, `vaga/[cargo]/[item]`, `estudo/[materia]/[item]` via `ItemEstudoView`). `MateriaConcurso.simuladoSlug` -> "Fazer teste". Testes ses-to: `prisma/seed-ses-to-testes.ts`. `limpar()` troca em-dash de conteúdo importado por hífen. **Prod: mudança de schema exige `prisma db push` no Neon + seed; mudança só de dados (dados/*.json) exige só o seed.**
